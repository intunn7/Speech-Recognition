...
class YukBicaraApp:
    def __init__(self, master):
        ...
        self.bg = PhotoImage(file="BG-01.png")
        ...
        self.result_display = Text(self.frame, height=5, width=30, wrap="word", font=("Josefin sans", 12), bg='#F0F0F0', fg='#333333')
        self.result_display.place(x=1000, y=453)
        self.result_display.config(highlightthickness=0, borderwidth=0)

        self.compare_page_button = Button(self.frame, text="Lihat Hasil", command=self.show_comparison_page, font=("Josefin sans", 12), bg='#F0F0F0', fg='#333333')
        self.compare_page_button.place(x=1115, y=600)
        self.compare_page_button.config(highlightthickness=0, borderwidth=0)

    def show_comparison_page(self):
        comparison_window = Toplevel(self.master)
        comparison_window.title("Hasil")

        reference_label = Label(comparison_window, text="Teks Referensi:", font=("Josefin sans", 12))
        reference_label.place(x=300, y=10)

        reference_text = Text(comparison_window, height=5, width=65, wrap="word", font=("Josefin sans", 12), bg='#F0F0F0', fg='#333333')
        reference_text.place(x=15, y=50)
        reference_text.insert("end", self.current_reference_text)

        result_label = Label(comparison_window, text="Teks Hasil Rekaman:", font=("Josefin sans", 12))
        result_label.place(x=280, y=230)

        result_text = Text(comparison_window, height=5, width=65, wrap="word", font=("Josefin sans", 12), bg='#F0F0F0', fg='#333333')
        result_text.place(x=15, y=280)
        result_text.insert("end", self.result_display.get("1.0", "end"))

        word_count_label_ref = Label(comparison_window, text="Jumlah Kata :", font=("Josefin sans", 12))
        word_count_label_ref.place(x=300, y=180)

        word_count_label_result = Label(comparison_window, text="Jumlah Kata :", font=("Josefin sans", 12))
        word_count_label_result.place(x=300, y=400)

        word_count_ref = len(self.current_reference_text.split())
        word_count_result = len(self.result_display.get("1.0", "end").split())

        word_count_value_ref = Label(comparison_window, text=str(word_count_ref), font=("Josefin sans", 12))
        word_count_value_ref.place(x=400, y=180)

        word_count_value_result = Label(comparison_window, text=str(word_count_result), font=("Josefin sans", 12))
        word_count_value_result.place(x=410, y=400)

        errors_label = Label(comparison_window, text="Kata-kata yang Salah:", font=("Josefin sans", 12))
        errors_label.place(x=15, y=450)

        errors_text = Text(comparison_window, height=5, width=65, wrap="word", font=("Josefin sans", 12), bg='#F0F0F0', fg='#333333')
        errors_text.place(x=15, y=490)
        errors_text.config(highlightthickness=0, borderwidth=0)

        errors = self.detect_errors(self.current_reference_text, self.result_display.get("1.0", "end"))
        if errors:
            for ref_word, hyp_word in errors:
                errors_text.insert("end", f"- Seharusnya {ref_word}, sedangkan yang anda ucapkan adalah {hyp_word}\n")
        else:
            errors_text.insert("end", "Tidak ada kesalahan.")
    ...
    def listen_and_process(self):
        try:
            ...
            with sr.Microphone() as source:
                ...
                self.speed_label.config(text=f"Kecepatan berbicara: {speech_rate:.2f} wpm")
                ...
                self.result_display.insert("end", f"{text}")
